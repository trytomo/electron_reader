...
public class UHFKillFragment extends Fragment implements View.OnClickListener {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    ...
                    if (TextUtils.isEmpty(data) || !data.matches(rex)) {
                        Toast.makeText(mContext, "The filtered data must be hexadecimal data", Toast.LENGTH_SHORT).show();
    ...
    public void kill() {
        String strPWD = EtAccessPwd_Kill.getText().toString().trim();
        ...
        if (cb_filter_kill.isChecked()) {
            ...
            if (filterData == null || filterData.isEmpty()) {
                Toast.makeText(mContext, "Filter data cannot be empty", Toast.LENGTH_SHORT).show();
                return;
            }
            if (etPtr_filter_kill.getText().toString() == null || etPtr_filter_kill.getText().toString().isEmpty()) {
                Toast.makeText(mContext, "Filter start address cannot be empty", Toast.LENGTH_SHORT).show();
                return;
            }
            if (etLen_filter_kill.getText().toString() == null || etLen_filter_kill.getText().toString().isEmpty()) {
                Toast.makeText(mContext, "Filter data length cannot be empty", Toast.LENGTH_SHORT).show();
                return;
            }
